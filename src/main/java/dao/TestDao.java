...
public class TestDao extends Dao {
    ...
    public Test get(String studentNo, String subjectCd, String schoolCd, int no) throws Exception {
        ...
        try {
            statement = connection.prepareStatement(
                ...
                " from test t" +
                " left join student s on t.student_no = TRIM(s.no)" +
    ...
    public List<Test> filterByStudent(String studentNo, School school) throws Exception {
        ...
        try {
            ...
            ResultSet rSet = statement.executeQuery();
            while (rSet.next()) list.add(mapRow(rSet));
        } catch (Exception e) {
            throw e;
        } finally {
            close(statement, connection);
    ...
    public List<Test> filterBySubjectAndClass(String subjectCd, String classNum, School school) throws Exception {
        ...
        PreparedStatement statement = null;
        try {
            String sql = "select t.*, s.name as student_name, s.ent_year as ent_year," +
                " sub.name as subject_name" +
                " from test t" +
                " left join student s on t.student_no = TRIM(s.no)" +
                " left join subject sub on t.subject_cd = sub.cd and t.school_cd = sub.school_cd" +
    ...
    private Test mapRow(ResultSet rSet) throws SQLException {
        ...
        try { test.setEntYear(rSet.getInt("ent_year")); } catch (SQLException ignored) {}
        return test;
    }
